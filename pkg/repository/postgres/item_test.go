...
func TestTodoItemPostgres_GetAll(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := r.GetAll(tt.input.listId)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
...
func TestTodoItemPostgres_GetById(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: model.TodoItem{Id: 1, Title: "title1", Description: "description1", Done: true},
		},
		{
			name: "Not Found",
			mock: func() {
...
func TestTodoItemPostgres_Delete(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := r.DeleteById(tt.input.userId, tt.input.itemId)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
